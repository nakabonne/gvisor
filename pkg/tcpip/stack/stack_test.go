	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.autoGen {
				...
				if addr.Address != lladdr {
					t.Fatalf("got stack.GetMainNICAddress(_, _) = %s, want = %s", addr, lladdr)
				}
			} else {
				// Should not have auto-generated an address.
				if addr, err := s.GetMainNICAddress(1, header.IPv6ProtocolNumber); err != tcpip.ErrNoLinkAddress {
					t.Fatalf("got stack.GetMainNICAddress(_, _) = (%s, %v), want = (, tcpip.ErrNoLinkAddress)", addr, err, lladdr)
				}
			}
		})
	}
