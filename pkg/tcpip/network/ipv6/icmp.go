...
func (e *endpoint) handleICMP(r *stack.Route, netHeader buffer.View, vv buffer.VectorisedView) {
	...
	case header.ICMPv6NeighborSolicit:
		...
		if err != nil {
			...
			// packet.
			//
			// TODO(ghanan): Handle this better?
			fmt.Println("ndpns: Invalid NICID for route = %d", rxNICID)
			return
		}
		...
		if isTentative {
			...
			if iph.SourceAddress() == header.IPv6Any {
				fmt.Println("ndpns: Duplicate tentative address (%s) is detected on NIC(%d)", targetAddr, rxNICID)
			...
			// NIC any further.
			fmt.Println("ndpns: Ending processing as target address (%s) is tentative on NIC(%d)", targetAddr, rxNICID)
			return
		}
	...
	case header.ICMPv6NeighborAdvert:
		...
		if err != nil {
			...
			//
			// TODO(ghanan): Handle this better?
			fmt.Println("ndpna: Invalid NICID for route = %d", rxNICID)
			return
		}
		...
		if isTentative {
			...
			// the NDP NA.
			fmt.Println("ndpna: Duplicate address (%s) is detected on NIC(%d)", targetAddr, rxNICID)
			stack.DupTentativeAddrDetected(rxNICID, targetAddr)
			return
		}
		...
		//
		// TODO(ghanan): Handle the scenario described above (inform the
		//               netstack integration that a duplicate address
		//               was detected?)
