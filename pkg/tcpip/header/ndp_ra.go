...
func (b NDPRouterAdvert) RouterLifetime() time.Duration {
	...
	return time.Second * time.Duration(binary.BigEndian.Uint16(b[ndpRARouterLifetimeOffset:][:2]))
}
...
func (b NDPRouterAdvert) ReachableTime() time.Duration {
	...
	return time.Millisecond * time.Duration(binary.BigEndian.Uint32(b[ndpRAReachableTimeOffset:][:4]))
}
...
func (b NDPRouterAdvert) RetransTimer() time.Duration {
	// The field is the time in milliseconds, as per RFC 4861 section 4.2.
	return time.Millisecond * time.Duration(binary.BigEndian.Uint32(b[ndpRARetransTimerOffset:][:4]))
}
