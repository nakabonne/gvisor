...
func doSplice(t *kernel.Task, outFile, inFile *fs.File, opts fs.SpliceOpts, nonBlocking bool) (int64, error) {
	...
	for opts.Length > 0 {
		t.Infof("doing splice on output(%+v), input(%+v), nonBlocking=%v, outFile.Flags().NonBlocking=%v", outFile, inFile, nonBlocking, outFile.Flags().NonBlocking)
		n, err = fs.Splice(t, outFile, inFile, opts)
		t.Infof("result is %v, %v", n, err)
		opts.Length -= n
		total += n
